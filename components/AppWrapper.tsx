      ...
      <aside className="w-64 border-r border-zinc-800 bg-zinc-950 flex flex-col p-6 fixed h-full z-40">
        ...
        <nav className="flex-1 space-y-2">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.name} href={item.href} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-mono text-xs tracking-widest uppercase transition-all ${isActive ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' : 'text-zinc-500 hover:text-zinc-200 hover:bg-zinc-900'}`}>
        ...
        <button onClick={() => supabase.auth.signOut()} className="flex items-center gap-3 px-4 py-3 text-zinc-500 hover:text-red-400 font-mono text-xs tracking-widest uppercase transition-colors">
